...
// Presence alone isn't enough — an expired cookie can still be sitting in
// the file with a non-empty value. expires is a Unix timestamp in seconds.
function isAuthCookieFresh(cookies) {
	const c = cookies.find((c) => c.name === AUTH_COOKIE_NAME && c.value);
	if (!c) return false;
	if (!c.expires) return true; // no expiry info at all — nothing to compare against
