...
async function tapByText(log, pattern) {
	try {
		...
		const xml = await adb(["-s", ADB_SERIAL, "shell", "cat", UI_DUMP_PATH], log);
		...
		for (const node of nodes) {
			...
			log("ANDROID", `Accessibility tree match "${text}" at (${cx}, ${cy}) — tapping via input tap.`);
			await adb(["-s", ADB_SERIAL, "shell", "input", "tap", String(cx), String(cy)], log);
			return true;
		}
...
async function connectAndroidChrome(log, signal) {
	...
	try {
		...
		// puppeteer.connect() has no built-in timeout — if the WebSocket
		// handshake to the device's Chrome stalls, it hangs forever (until
		// the overall run timeout fires minutes later). Same for
		// browser.pages(), which does a CDP round-trip. Bound both, and
		// retry once, since a stale forward from a previous run is a
		// common and self-correcting cause.
		...
		for (let attempt = 1; attempt <= 2; attempt++) {
			try {
				log("ANDROID", `Connecting to Chrome DevTools (attempt ${attempt})...`);
				...
				const pages = await withLimit(browser.pages(), 15000, "browser.pages");
				page = pages.find((p) => p.url().includes("pixai.art")) || pages[0] || (await browser.newPage());
				log("ANDROID", "Connected to Chrome.");
				lastErr = null;
				break;
			} catch (e) {
				lastErr = e;
				...
				browser = undefined;
				if (attempt < 2) {
					// Re-establish the forward in case it went stale.
					try {
						await adb(["-s", ADB_SERIAL, "forward", "--remove", `tcp:${DEVTOOLS_LOCAL_PORT}`], log);
					} catch (_) {}
					await adb(["-s", ADB_SERIAL, "forward", `tcp:${DEVTOOLS_LOCAL_PORT}`, "localabstract:chrome_devtools_remote"], log);
...
module.exports = { connectAndroidChrome, disconnectAndroidChrome, forceSleepScreen, swipeUp, tapByText };
