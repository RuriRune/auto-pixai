	...
	try {
		...
		try {
			...
			if (match) {
				...
				await adb(
					["-s", ADB_SERIAL, "shell", "input", "swipe", String(x), String(Math.round(h * 0.8)), String(x), String(Math.round(h * 0.2)), "300"],
					log
				);
				await new Promise((r) => setTimeout(r, 500));
			}
...
module.exports = { connectAndroidChrome, disconnectAndroidChrome, forceSleepScreen };
