...
async function connectAndroidChrome(log, signal) {
	...
	try {
		...
		// puppeteer.connect() has no built-in timeout, so bound it — but
		// generously. This device is genuinely slow (the Daily Claim modal
		// alone can take over a minute), so an aggressive timeout here just
		// kills a connection that was progressing fine. No retry either:
		// tearing down and reconnecting mid-handshake is more likely to
		// break a slow-but-working connection than to fix a real hang, and
		// a real hang is already caught by the overall run timeout.
		...
		log("ANDROID", "Connecting to Chrome DevTools (this can take a while on a slow device)...");
		...
		const pages = await withLimit(browser.pages(), 60000, "browser.pages");
