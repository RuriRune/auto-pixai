...
async function connectAndroidChrome(log) {
	...
	// KEYCODE_WAKEUP wakes the display if it's asleep and is a no-op if
	// it's already on (unlike the POWER key, which toggles). With no
	// screen lock set on the device, Chrome is immediately usable — no
	// swipe/unlock step needed. The phone can keep a normal short screen
	// timeout the rest of the time.
	log("ANDROID", "Waking the screen...");
